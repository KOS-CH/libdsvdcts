...
import {
    ...
    outputDescription,
    outputSetting,
    createSubElements, globalHelperItem,
} from "./messageMapping";
 ...
 */
export function _vdcResponseGetProperty(this: libdsvdc, conn: any, decodedMessage: any) {
            ...
            decodedMessage.vdsmRequestGetProperty.query.forEach((p: any) => {
                ...
                if (p.name == "outputSettings") {
                    ...
                    if (device.outputSetting) {
                        device.outputSetting.forEach((desc: { [key: string]: string, value:any }) => {
                            // loop all keys of an object
                            elements = [];

                            for (const [key, value] of Object.entries(desc)) {
                                if (key && outputSetting.find((o: globalHelperItem) => o.name == key)) {
                                    if (
                                        outputSetting && outputSetting.find((o:globalHelperItem) => o.name == key)?.type ==
                                        "elements"
                                    ) {
                                        ...
                                        const valObj: any = {};
                                        const keyObj = outputSetting.find((o:globalHelperItem) => o.name == key)
                    ...
                    if (device.outputDescription) {
                        device.outputDescription.forEach((desc: { [key: string]: string; value: any}) => {
                            // loop all keys of an object
                            elements = [];
                    ...
                    if (device.sensorDescription) {
                        ...
                        device.sensorDescription.forEach((desc: { [key: string]:string; value: any}) => {
 ...
 * @param {*} obj
 */
export function _vdcSendPushProperty(this: libdsvdc, conn: any, message = {
    obj: undefined,
    dSUID: ""
}) {
 ...
 */
export function _vdsmResponseHello(this: libdsvdc, conn: any, decodedMessage: any) {
    const answerObj = this.vdsm.fromObject({
        type: 3,
        messageId: decodedMessage.messageId,
 ...
 */
export function _genericResponse(this: libdsvdc, conn: any, GenericResponse: any, messageId:any) {
    // this.messageId = this.messageId + 1;
 ...
 */
export function _vdcSendAnnounceDevice(this: libdsvdc, conn:any, dSUID:string) {
    this.messageId = this.messageId + 1;
    const answerObj = this.vdsm.fromObject({
        type: 10,
        messageId: this.messageId,
